...
#include"ibex_functional_coverage.h"
    ...
    void FunctionalCoverage::initialize_delay_map() {
        std::vector<int> gnt = {0, 1, 2, 3, 4, 5};
        std::vector<int> rvalid = {0, 1, 2, 3, 4, 5};
    
        // Nested loops to define your total cross-coverage space
        for(int g : gnt) {
            for(int v : rvalid) {
                // Pre-seed the map with 0 hits so they show up in the final report
                cross_delay_bin[std::make_tuple(g, v)] = 0; 
            }
        }
    }

    void FunctionalCoverage::sample_coverage(bool is_compressed, bool is_misaligned, int gnt_delay, int rvalid_delay) {
        
        if (!map_instantiated) {
            FunctionalCoverage::initialize_delay_map();
            map_instantiated = true;
        }
        ...
        auto key = std::make_tuple(gnt_delay, rvalid_delay);
        cross_delay_bin[key] += 1;
    }
    ...
    void FunctionalCoverage::report_coverage() {
        ...
        std::cout << "[Cross-Coverage] Combinations (gnt_delay x rvalid_delay):\n";
        for (const auto& [combo, count] : cross_delay_bin) {
            const auto& [gnt, rvalid] = combo;
            
            std::cout << "HIT [" << count << " times] -> "
                      << "gnt: " << gnt 
                      << " | rvalid: " << rvalid << "\n";
        }
        std::cout << "===================================================\n\n";
