...
int main(int argc, char** argv){
    ...
    uint32_t master_seed = contextp->randSeed();
    ...
    MemoryDriver* driver = new MemoryDriver(dut, monitor, vmem_path, master_seed);
    ...
    while (!contextp->gotFinish() && contextp->time() < 10000) {
        ...
        // A. ALWAYS run components so pins are actively driven!
        for (auto comp : tb_env) {
            comp->run(); 
        }

        // B. [FIX]: Squash transient garbage during hardware flush
        if (tb_cooldown > 0) {
            tb_cooldown--; 
            
            // If the hardware is flushing, wipe any garbage the driver 
            // just accidentally queued and force its output pins back to 0!
            for (auto comp : tb_env) {
                comp->reset(); 
            }
        }
        
        // Evaluate the final overridden pin states
        dut->eval(); 
